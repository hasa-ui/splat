...
- 2026-04-10: Fixed the follow-up control regression from the camera refactor by basing move/aim projection on the currently rendered camera forward vector instead of the player's desired facing, so camera-relative controls stay aligned during smoothing.
- 2026-04-10: Fixed the remaining pause/resume camera lurch by pre-applying the gameplay camera pose at the moment the match re-enters `playing`, so the first resumed input frame no longer uses the stale overview basis.
- 2026-04-10: Tightened the resume fix so `enterGameplayMode()` now snaps the rendered camera itself before input is re-enabled, and the pause overlay Resume button now routes through the same helper instead of bypassing the camera sync.
